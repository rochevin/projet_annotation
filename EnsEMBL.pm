...
#Module qui se connecte à la base de donnée EnsEMBL

#Connexion à la base de données EnSEMBL
#en utilisant le module API d'EnsEMBL
#Input : Tableau de donnée dataset + liste des id Gene
#Output : %dataset qui contient : ID gene,ID transcrits + proteine (canonique et les autres)
sub get_ensembl_id {

	my ($dataset,$gene_id)=@_;
	#On récupère le tableau associatif et la liste, qu'on attribue a des variables locales
	my %dataset=%$dataset;
	my @gene_id=@$gene_id;
	#On définit un nouvel objet de type Bio::EnsEMBL::Registry
